...
function inferCapabilities(id, raw = {}) {
  const text = `${id} ${raw.id || ''} ${raw.name || ''} ${raw.description || ''} ${raw.owned_by || ''} ${raw.type || ''} ${raw.category || ''}`.toLowerCase();
  ...
  if (/free|free_endpoint|free endpoint/.test(text)) caps.add('free');
  if (/paid|credit|partner|premium/.test(text)) caps.add('paid');
...
function badgeLabel(cap) {
  const map = {
    chat: '💬 Chat', reasoning: '🧠 Reasoning', coding: '💻 Coding', research: '📚 Research', vision: '👁 Vision', image: '🖼️ Image', speech: '🎤 Speech', long: '📄 Long', fast: '⚡ Fast', free: '🟢 Free', paid: '💳 Paid', enterprise: '🏢 Enterprise', live: 'Live'
  };
  return map[cap] || cap;
}

function capabilityHtml(model) {
  const order = ['reasoning', 'coding', 'research', 'vision', 'image', 'speech', 'long', 'fast', 'free', 'paid', 'enterprise', 'live'];
...
function renderChatHistory() {
  ...
  el.innerHTML = state.chats.slice(0, 40).map(chat => `
    <div class="chat-history-item ${state.currentChat?.id === chat.id ? 'active' : ''}" onclick="selectChat('${chat.id}')">
...
function messageHtml(m, idx) {
  ...
  const content = m.loading && !m.content ? thinkingHtml() : renderMarkdown(m.content || '');
...
function thinkingHtml() {
  ...
  return `<span class="thinking-line">Thinking <span class="thinking-dots"><span></span><span></span><span></span></span></span>`;
}
...
async function maybeBuildWebSearchContext(assistantId) {
  ...
  if (msg && state.settings.showThinking) {
    msg.thinking += `Web Search plugin: searching ${state.settings.plugins.webSearchProvider} for current results...\n`;
  ...
  try {
    ...
    if (msg && state.settings.showThinking) {
      msg.thinking += `Web Search plugin: found ${results.length} result(s).\n`;
      updateAssistantDom(msg);
    }
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (delta.content || delta.thinking) {
          sawChunk = true;
          const msg = getMessage(assistantId); if (!msg) continue;
          if (delta.thinking) msg.thinking += delta.thinking;
          if (delta.content) msg.content += delta.content;
          updateAssistantDom(msg);
        }
      } catch (_) {
        if (!mayFail) {
          const msg = getMessage(assistantId); if (msg) { msg.content += payload; updateAssistantDom(msg); sawChunk = true; }
        }
      }
    }
  }
...
function extractDelta(json) {
  ...
  return {
    content: delta.content || delta.text || json.output_text || '',
...
function updateAssistantDom(msg) {
  ...
  if (body) {
    const thinking = msg.thinking ? `<div class="thinking-block expanded"><div class="thinking-header"><span>🧠</span><div class="thinking-title">Thinking</div></div><div class="thinking-body" style="display:block;">${escapeHtml(msg.thinking)}</div></div>` : '';
    body.innerHTML = thinking + (msg.content ? renderMarkdown(msg.content) : thinkingHtml());
  }
  scrollToBottom(true);
}
...
function updateStatus() {
  ...
  if (avatar) avatar.textContent = (state.settings.userName || 'U').slice(0, 1).toUpperCase();
...
function openPanel(title, html) { document.getElementById('panelTitle').textContent = title; document.getElementById('panelBody').innerHTML = html; document.getElementById('panelOverlay').classList.add('open'); document.getElementById('sidePanel').classList.add('open'); }
...
function openAgentPanel() {
  openPanel('Agent Swarm', `<p style="font-size:12px;color:var(--text-secondary);line-height:1.5;margin-bottom:12px;">Agents are real now: the selected agent adds an extra system prompt to every message.</p>${AGENTS.map(a => `<div class="agent-card ${state.settings.currentAgent === a.key ? 'selected' : ''}" onclick="selectAgent('${a.key}')"><div class="agent-header"><div class="agent-avatar">${a.emoji}</div><div><div class="agent-name">${escapeHtml(a.name)}</div><div class="agent-role">${escapeHtml(a.role)}</div></div></div><div class="agent-desc">${escapeHtml(a.prompt || 'No extra prompt. Balanced default behaviour.')}</div></div>`).join('')}`);
}
...
function exportToPDF() {
  ...
  const html = `<html><head><title>${escapeHtml(state.currentChat.title)}</title><style>body{font-family:Arial,sans-serif;padding:32px;line-height:1.5}pre{white-space:pre-wrap;background:#f2f2f2;padding:12px;border-radius:8px}.role{font-weight:bold;margin-top:18px}</style></head><body><h1>${escapeHtml(state.currentChat.title)}</h1>${state.currentChat.messages.map(m => `<div class="role">${escapeHtml(m.role)}</div><div>${renderMarkdown(m.content || '')}</div>`).join('')}<script>print()<\/script></body></html>`;
...
async function shareChat() {
  const text = state.currentChat?.messages.map(m => `${m.role.toUpperCase()}: ${m.content}`).join('\n\n') || '';
