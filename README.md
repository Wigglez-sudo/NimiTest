...
| Version | Theme |
| --- | --- |
| `v3.3.4` | Broader audit pass: duplicate finish-reason handler removed, File Reader gating fixed for images and manifests, markdown paragraph/table rendering improved, mono font token added, and mobile model-tab overflow smoothed. |
...
- This `v3.3.4` release changes frontend files only. Cloudflare Worker redeploy is not required.
...
git commit -m "Release v3.3.4 audit cleanup pass"
git push
```
...
No Worker change is required for `v3.3.4`.
