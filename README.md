...
| Version | Theme |
| --- | --- |
| `v3.2.2` | Runtime freeze guard for generated file responses, iOS wrapping, chat delete touch targets, model picker refinement, username cleanup. |
...
- Bumped service-worker cache to `nvidia-ai-desktop-v3-2-2`.
...
- This `v3.2.2` release changes frontend files only. Cloudflare Worker redeploy is not required.
...
git commit -m "Release v3.2.2 runtime freeze guard"
git push
```
...
```text
https://wigglez-sudo.github.io/nvidia-ai-desktop/?v=3.2.2
```

### Cloudflare Worker

No Worker change is required for `v3.2.2`.
